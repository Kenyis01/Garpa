...
export async function createGroup(params: {
  ...
  const { error: mErr } = await supabase
    .from('group_members')
    .insert(
      allMembers.map((uid) => ({ group_id: group.id, user_id: uid, role: 'member' as const })),
    );
...
export async function getGroupMembers(
  ...
  const balances = await Promise.all(
    userIds.map((uid) =>
      uid === currentUserId
        ? Promise.resolve(0)
        : calcPairBalanceInGroup(currentUserId, uid, groupId),
    ),
  );
...
async function calcPairBalanceInGroup(
  userId: string,
  otherId: string,
  groupId: string,
): Promise<number> {
