...
export const Button = ({ title, variant = 'primary', isLoading, style, ...props }: ButtonProps) => {
  const getBackgroundColor = () => {
    ...
    switch (variant) {
      ...
      case 'danger': return Colors.brand.orange; // O el rojo de danger
  ...
  return (
    <TouchableOpacity
      style={[
        styles.container,
        { backgroundColor: getBackgroundColor(), borderWidth: variant === 'outline' ? 1 : 0, borderColor: Colors.brand.primary },
        style,
      ]}
      activeOpacity={0.7}
      {...props}
