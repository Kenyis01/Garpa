...
export default function ActivityScreen() {
  ...
  const loadActivity = useCallback(async () => {
    ...
    try {
      ...
      // Get group names
      const groupIds = [
        ...new Set(expenses.map((e) => e.group_id).filter((g): g is string => Boolean(g))),
      ];
  ...
  function renderItem({ item }: { item: ActivityItem }) {
    ...
    return (
      ...
      >
        <View
          style={[styles.itemIcon, item.type === 'settlement' && { backgroundColor: '#ecfdf5' }]}
        >
          <MaterialCommunityIcons
            name={icon as any}
            size={20}
            color={item.type === 'settlement' ? '#10b981' : '#6b7280'}
          />
        </View>
